...
public interface PriceModelLevelRepository extends JpaRepository<PriceModelLevel, Long>, JpaSpecificationExecutor<PriceModelLevel> {
    boolean existsByWeightBandEndAndPricingLevelLevelIdAndRegionRegionId(BigDecimal weightBandEnd, Long pricingModelId, Long regionId);
    @Query("SELECT CASE WHEN COUNT(pml) > 0 THEN true ELSE false END " +
            ...
            "AND ((:weightBandStart BETWEEN pml.weightBandStart AND pml.weightBandEn" +
            "d) " +
            "OR (:weightBandEnd BETWEEN pml.weightBandStart AND pml.weightBandEnd) " +
            "OR (pml.weightBandStart BETWEEN :weightBandStart AND :weightBandEnd) " +
            "OR (pml.weightBandEnd BETWEEN :weightBandStart AND :weightBandEnd))" +
    ...
    @Query("SELECT pml FROM PriceModelLevel pml JOIN pml.pricingLevel pm WHERE pml.shippingService.serviceId = :servicePortfolioId AND (:weightToUse BETWEEN pml.weightBandStart AND pml.weightBandEnd OR pml.weightBandStart = :weightToUse OR pml.weightBandEnd = :weightToUse) AND pml.region = :region AND pm.levelId = :pricingCategory")
